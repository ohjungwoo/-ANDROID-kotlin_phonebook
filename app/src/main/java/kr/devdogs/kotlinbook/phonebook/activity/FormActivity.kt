...
class FormActivity : AppCompatActivity() {
    ...
    private fun initView() {
        ...
        submitBtn?.setOnClickListener {
            ...
            if ("" == name || "" == phone) {
                Toast.makeText(this@FormActivity, "이름, 휴대폰은 필수입니다", Toast.LENGTH_SHORT)
            ...
            realm?.executeTransaction { realm ->
                if (mode == MODE_INSERT) {
                    ...
                    val currentIdNum = realm.where<PhoneBook>(PhoneBook::class.java).max("id")
    ...
    private fun setMode() {
        ...
        if (mode == MODE_UPDATE) {
            ...
            if (currentPhoneBook!!.photoSrc == null) {
                photoView?.setImageDrawable(getDrawable(R.drawable.icon_man))
            } else {
                photoView?.setImageBitmap(BitmapFactory.decodeFile(Uri.parse(currentPhoneBook!!.photoSrc).path))
            }
