...
class PhoneBookListAdapter(context: Context, private val viewId: Int, var items: List<PhoneBook>?) : ArrayAdapter<PhoneBook>(context, viewId, items) {
        ...
        items?.let {
            ...
            if (item != null) {
                ...
                if (item.photoSrc == null) {
                    photoView.setImageDrawable(context.getDrawable(R.drawable.icon_man))
                } else {
                    photoView.setImageBitmap(BitmapFactory.decodeFile(Uri.parse(item.photoSrc).path))
                }
